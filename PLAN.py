...
INFO_QUESTIONS = [
    ...
    "29.是否提及 AI 應用應遵循的七大倫理原則（自主、透明、當責、安全、公平、永續、隱私）？",
...
def build_individual_prompts(questions, full_text, role_name="審查委員"):
    ...
    for q in questions:
        ...
        prompt = f"""
你是一位{role_name}，請參考下方「智慧醫療中心技術手冊」的審查規範，針對指定題目審查下方文件內容。  
若文件未明確提及該項內容，請勿推論、延伸或合理補足，並依規定回報 0 分與具體原因。

📌 評分格式請嚴格依照下列格式（否則系統無法解析）：
得分 ⟪x⟫/1 ，原因：...(請緊接在同一行)
